...
\begin{enumerate}
  ...
  \item \textbf{系统与效率（2020--2023）}：Flash Attention~\cite{dao2022flashattention} 通过消除显存瓶颈让训练加速 2--4$\times$。LoRA~\cite{hu2022lora} 让在单个节点上微调 70B+ 模型成为可能。专家混合（Mixture-of-Experts, MoE）将模型容量与算力成本解耦。vLLM 这类推理引擎把吞吐量带到了实时应用可触及的范围。
...
\begin{itemize}
  ...
  \item \textbf{理解 LLM 内部机制}——注意力机制、位置编码、MoE 路由、Flash Attention，以及架构选择为何会影响下游能力。
...
\begin{enumerate}
  ...
  \item \textbf{第一部分——基础}（第 1--3 章）：LLM 架构与优化（Transformer、attention、位置编码、Flash Attention、LoRA、MoE）、系统基础（GPU 层级、分布式训练、vLLM），以及经典 RL 理论（MDP、policy gradient、actor-critic）。
...
\begin{itemize}
  ...
  \item \textbf{第 1 章——LLM 架构与优化}：Transformer 内部机制（self-attention、multi-head attention、RoPE、GQA）、Flash Attention、优化方法（AdamW、学习率调度、梯度裁剪）、混合精度、LoRA/QLoRA、量化、知识蒸馏与 Mixture of Experts。
